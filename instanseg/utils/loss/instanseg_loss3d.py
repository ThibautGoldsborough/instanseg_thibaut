...
def find_all_local_maxima_3d(volume: torch.Tensor, neighbourhood_size: int, minimum_value: float) -> torch.Tensor:
    """
    Finds all the local maxima within each neighbourhood in a 3D volume.
    May return multiple maxima per neighbourhood.
    """
    # Set kernel size for 3D pooling
    kernel_size = 2 * neighbourhood_size + 1
    pooled = F.max_pool3d(volume, kernel_size=kernel_size, stride=1, padding=kernel_size // 2)

    # Create a mask for local maxima
    mask = (pooled == volume) * (volume >= minimum_value)

    # Apply mask to retain only the local maxima values
    local_maxima = volume * mask

    return local_maxima

def torch_peak_local_max_3d(volume: torch.Tensor, neighbourhood_size: int, minimum_value: float,
                            return_map: bool = False, dtype: torch.dtype = torch.int) -> torch.Tensor:
    """
    Finds local maxima in a 3D volume tensor. Uses the first max found in the neighbourhood.
    """
    d, h, w = volume.shape
    volume = volume.view(1, 1, d, h, w)
    device = volume.device

    kernel_size = 2 * neighbourhood_size + 1
    pooled, max_inds = F.max_pool3d(volume, kernel_size=kernel_size, stride=1, padding=neighbourhood_size,
                                    return_indices=True)

    inds = torch.arange(0, volume.numel(), device=device, dtype=dtype).reshape(volume.shape)
    ...
    # Use torch.nonzero to return indices where peak local max is true
    return torch.nonzero(peak_local_max.squeeze()).to(dtype)
...
def generate_random_3d_slice(depth, height, width, device, return_volume = False):
    ...
    # Stack to form (x, y, z) coordinates in each voxel of the 3D grid
    xyz_grid = torch.stack((xx, yy, zz), dim=0)  # Shape: (3, depth, height, width)

    input = xyz_grid[None]
    aug = RandomRotation3D(90, p=1.)
    out = aug(input)
    out = out[:,:,out.shape[-3] // 4: 3 * out.shape[-3] // 4, out.shape[-2] // 4: 3 * out.shape[-2] // 4, out.shape[-1] // 4: 3 * out.shape[-1] // 4]
    from torch.nn.functional import interpolate
    out =interpolate(out, size=(256, 256, 256), mode='trilinear', align_corners=False)
    
    slice_grid = out[0,:,out.shape[-3] // 2]*2
    
    return slice_grid



def generate_coordinate_map(mode: str = "linear", spatial_dim: int = 2, height: int = 256, width: int = 256, device: torch.device = torch.device(type='cuda'), **kwargs) -> torch.Tensor:
    ...
    if mode == "linear":
        if spatial_dim ==2:
            ...
            xxyy = generate_random_3d_slice(height, height, width, device, **kwargs)
...
def feature_engineering_slow_3d(x: torch.Tensor, c: torch.Tensor, sigma: torch.Tensor, window_size: int,
                        mesh_grid_flat: torch.Tensor):
    x = x.squeeze(0)
    E = x.shape[0]
    d, h, w = x.shape[-3:]
  ...
  #  pdb.set_trace()
    ...
    x_slices = x[:, :, mesh_grid_flat[0], mesh_grid_flat[1]].reshape(
        E, C, d, 2 * window_size, 2 * window_size).permute(1, 0, 2, 3, 4)  # C,E,2*window_size,2*window_size,2*window_size
    sigma_slices = sigma[:, :, mesh_grid_flat[0], mesh_grid_flat[1]].reshape(
        S, C, d, 2 * window_size, 2 * window_size).permute(1, 0, 2, 3, 4)  # C,S,2*window_size,2*window_size,2*window_size
   # pdb.set_trace()

    c_shaped = c.reshape(-1, E, 1, 1, 1)

    diff = x_slices - c_shaped
    x = torch.cat([diff, sigma_slices], dim=1)  # C, E+S, D, H, W

    x = x.flatten(2).permute(0, -1, 1)  # C, D*H*W, E+S
    x = x.reshape((x.shape[0] * x.shape[1]), x.shape[2])  # C*D*H*W, E+S
...
class InstanSeg3D(nn.Module):
    ...
    def postprocessing(self, prediction: Union[torch.Tensor, np.ndarray],
        ...
        for i in range(iterations):
            ...
            if precomputed_crops is None:
                ...
                depth, height, width = prediction_i.size(1), prediction_i.size(2), prediction_i.size(3)

                xxyy = generate_coordinate_map(mode = "linear", spatial_dim = self.dim_coords, height = height, width = width, device = device, return_volume=True)
               # pdb.set_trace()
              #  xxyy = xxyy[:,None].repeat(1,1,depth,1,1)[0]
                ...
                if precomputed_seeds is None:
                    local_centroids_idx = torch_peak_local_max_3d(mask_map, neighbourhood_size=int(peak_distance), minimum_value=seed_threshold)
                else:
                ...
                fields = fields + xxyy
                if self.to_centre:
                    fields_at_centroids = xxyy[:, local_centroids_idx[:, 0], local_centroids_idx[:, 1],local_centroids_idx[:, 2]]
                else:
                    fields_at_centroids = fields[:, local_centroids_idx[:, 0], local_centroids_idx[:, 1],local_centroids_idx[:, 2]]
                ...
                for z_idx in range(fields.shape[1]):

                    mask_map_tmp = mask_map[z_idx]
                    fields_tmp = fields[:,z_idx]
                    sigma_tmp = sigma[:,z_idx]

                    C = fields_at_centroids.shape[1]

                    h, w = mask_map_tmp.shape[-2:]
                    window_size = min(window_size, h, w)
                    window_size = window_size - window_size % 2

                    if C == 0:
                        label = torch.zeros(mask_map_tmp.shape, dtype=int, device=mask_map_tmp.device).squeeze()
                        labels.append(label)
                        continue

                    #torch.cuda.synchronize()
                    crops, coords = compute_crops(fields_tmp, 
                                                    fields_at_centroids.T, 
                                                    sigma_tmp, 
                                                    local_centroids_idx[:,1:].int(), 
                                                    feature_engineering = self.feature_engineering,
                                                    pixel_classifier=self.pixel_classifier,
                    ...
                    label_tmp = merge_sparse_predictions(crops, coords, mask_map_tmp, size=(C,h, w), mask_threshold=mask_threshold, window_size=window_size, min_size=min_size, overlap_threshold=overlap_threshold, mean_threshold=mean_threshold).int() #about 30% of the time
                    
                    label[z_idx] = label_tmp
    ...
    def postprocessing_v2(self, prediction: Union[torch.Tensor, np.ndarray],
                        mask_threshold: float = 0.53,
                        peak_distance: int = 5,
                        seed_threshold: float = 0.8,
                        overlap_threshold: float = 0.3,
        ...
        for i in range(iterations):
            ...
            if precomputed_crops is None:
                ...
                crops = torch.zeros(Z,C, 1, window_size, window_size, device = device, dtype = torch.float32)
                coords = torch.zeros(Z, 3, C * (window_size ** 2), device = device, dtype = torch.int32)
                ...
                for z_idx in range(fields.shape[1]):
                    ...
                    #torch.cuda.synchronize()
                    crops_tmp, coords_tmp = compute_crops(fields_tmp, 
                                                    fields_at_centroids.T, 
                                                    sigma_tmp, 
                                                    local_centroids_idx[:,1:].int(), 
                                                    feature_engineering = self.feature_engineering,
                    ...
                    #pdb.set_trace()
                    coords[z_idx] = coords_tmp
                    crops[z_idx] = crops_tmp
                ...
                crops = rearrange(crops, 'Z C 1 H W -> C 1 H (Z W)')

                mask_map = rearrange(mask_map, 'Z H W -> H (Z W)')

                coords_r = rearrange(coords, 'Z D (C H W) -> Z D C H W', W = window_size, H = window_size, D = 2)

                pdb.set_trace()

              #  for i in range(fields.shape[1]):
              #      coords_r[i, 1, :, :, :] += i * window_size

                coords_r = coords[0]

                coords = rearrange(coords_r, 'D C H W -> D (C H Z W)')

                #want 

                pdb.set_trace()

                #coords is shape Z, 3, C * (window_size ** 2)

                h, w = mask_map.shape[-2:]

                label_tmp = merge_sparse_predictions(crops, coords, mask_map, size=(C,h, w), mask_threshold=mask_threshold, window_size=window_size, min_size=min_size, overlap_threshold=overlap_threshold, mean_threshold=mean_threshold).int() #about 30% of the time
                pdb.set_trace()
                label[z_idx] = label_tmp
            ...
            for image_index in range(x_full.shape[0]):
                ...
                for i in iterations:
                    ...
                    if not self.to_3d:
                        ...
                        depth,height,width = x.size(1), x.size(2),x.size(3)
                        xxyy = generate_coordinate_map(mode = "linear", spatial_dim = self.dim_coords, height = height, width = width, device = x.device)
                    ...
                    if not self.to_3d:
                        centroids_idx = torch_peak_local_max(mask_map, 
                                                            neighbourhood_size=peak_distance,
                                                            minimum_value=seed_threshold, 
                                                            dtype= self.index_dtype)
                    else:
                        centroids_idx = torch_peak_local_max_3d(mask_map, 
                                    neighbourhood_size=peak_distance,
                                    minimum_value=seed_threshold, 
                                    dtype= self.index_dtype)

                        xxyy = xxyy[:,None].repeat(1,1,x.size(1),1,1)  #this has to be revisited.
                    ...
                    if not self.to_3d:
                        ...
                        if self.to_centre:
                            fields_at_centroids = xxyy[:,:, centroids_idx[:, 0], centroids_idx[:, 1],centroids_idx[:, 2]]
                        else:
                            fields_at_centroids = fields[:, :,centroids_idx[:, 0], centroids_idx[:, 1],centroids_idx[:, 2]]
                    ...
                    if not self.to_3d:
                        ...
                        mesh_grid_flat = torch.flatten(mesh_grid, 2).permute(1, 0, -1)  # 2,C,2*window_size*2*window_size
                        offset_from_origin = window_slices[:, 0].permute(1, 0)[:, :, None] # 2, C, 1
                        mesh_grid_flat = mesh_grid_flat + offset_from_origin
                        mesh_grid_flat = torch.flatten(mesh_grid_flat, 1)  # 2,C*2*window_size*2*window_size
                        ...
                        grid_x, grid_y = torch.meshgrid(
                            torch.arange(slice_size, device=x.device, dtype=self.index_dtype),
                            torch.arange(slice_size * d, device=x.device, dtype=self.index_dtype), indexing="ij")

                        pdb.set_trace()


                        mesh = torch.stack((grid_x, grid_y))
                        mesh_grid = mesh.expand(C, 2, slice_size, slice_size * d)
                        mesh_grid_flat = torch.flatten(mesh_grid, start_dim=2).permute(1, 0, -1)  # 2, C, slice_size^2
                        offset_from_origin = window_slices[:, 0].permute(1, 0)[:, :, None] # 2, C, 1
                        mesh_grid_flat = mesh_grid_flat + offset_from_origin
                        mesh_grid_flat = torch.flatten(mesh_grid_flat, 1)  # 2,C*2*window_size*2*window_size

                        x = feature_engineering_slow_3d(x, c, sigma, torch.tensor(window_size).int(), mesh_grid_flat)
                        x = torch.sigmoid(self.pixel_classifier(x))
                        
                        x = x.reshape(C, 1, depth, slice_size, slice_size)

                        x = x.flatten(2,3) #C,1,depth*slice,slice
...
if __name__ == "__main__":
   ...
   # from instanseg.utils.utils import export_to_torchscript
    ...
    model_2d,model_dict = load_model(model_name,path = "/home/cdt/Documents/Projects/InstanSeg/instanseg_thibaut/instanseg/models/") 

    from instanseg.utils.loss.instanseg_loss3d import InstanSeg3D
    method = InstanSeg3D(binary_loss_fn_str=model_dict["binary_loss_fn"], seed_loss_fn=model_dict["seed_loss_fn"],
                        n_sigma=model_dict["n_sigma"],
                        cells_and_nuclei=model_dict["cells_and_nuclei"], to_centre=model_dict["to_centre"],
                        window_size=64, dim_coords=model_dict["dim_coords"],
                        feature_engineering_function=model_dict["feature_engineering"])

    method.initialize_pixel_classifier(model_2d)


    model_2d.eval()

    model_3d = SoftACSConverter(model_2d).eval()

    #out_3d = model_3d(torch.randn(1,1,32,32,32))

    import numpy as np
    img = '/home/cdt/Downloads/Final Display of RL with 11 stacks 16 bit.tif'
    #img = '/home/cdt/Downloads/1_dapi_test_raw.tif'
    from tifffile import tifffile
    img = tifffile.imread(img)

    img3d = torch.nn.functional.interpolate(torch.tensor(img.astype(np.int32))[None,None].float(), size = (16,128,128), mode = "trilinear")
    img3d = img3d / img3d.max()


    #show_images(img3d[:,:,:,0])

    out3d = model_3d(img3d)

    model_2d.eval()

    model_3d = SoftACSConverter(model_2d).eval().to("cuda")

    #out_3d = model_3d(torch.randn(1,1,32,32,32))

    import numpy as np
    img = '/home/cdt/Downloads/ground_truth_and_synthetic/BABB-cleared_kidney_1/BABB-cleared_kidney_1-153-85/BABB-cleared_kidney_1-153-85.tif'
    #img = '/home/cdt/Downloads/1_dapi_test_raw.tif'
    from tifffile import tifffile
    img = tifffile.imread(img)

    img3d = torch.nn.functional.interpolate(torch.tensor(img.astype(np.int32))[None,None].float(), size = (128,128,128), mode = "trilinear")
    img3d = img3d / img3d.max()
    ...
    with torch.no_grad():
        ...
        out3d = model_3d(img3d.to("cuda"))

        out = method.postprocessing(out3d[0].to("cuda"), device = "cuda", window_size = 32)
    ...
    os.environ['INSTANSEG_MODEL_PATH'] = "../../models"
    os.environ['INSTANSEG_TORCHSCRIPT_PATH'] = "../../torchscripts"
    os.environ['EXAMPLE_IMAGE_PATH'] = "../../examples"
    ...
    def export_to_torchscript(model_str: str, show_example: bool = False, output_dir: str = "../torchscripts",
        ...
        from instanseg.utils.utils import set_export_paths
        ...
        input_data = tifffile.imread(os.path.join(example_path,"3d_dapi.tif"))
        ...
        input_tensor = torch.nn.functional.interpolate((input_tensor[None,None]), size = (16,128,128))[0]
